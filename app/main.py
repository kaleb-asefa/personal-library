...
@app.get('/api/books', response_model=list[books])
def api_books():
    return get_all_books()

@app.get('/api/books/{book_id}', response_model=books)
def api_book_detail(book_id: int):
    with get_session() as session:
        book = session.execute(select(Book).where(Book.book_id == book_id).options(joinedload(Book.author), joinedload(Book.genres))).unique().scalar_one_or_none()
        if book:
            return book
